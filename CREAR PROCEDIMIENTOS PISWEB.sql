...
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'usp_ObtenerPerfiles')
DROP PROCEDURE usp_ObtenerPerfiles

go

IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'usp_RegistrarPerfil')
DROP PROCEDURE usp_RegistrarPerfil

go

IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'usp_ModificarPerfiles')
DROP PROCEDURE usp_ModificarPerfiles

go
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'usp_EliminarPerfil')
DROP PROCEDURE usp_EliminarPerfil
...
begin
 ...
 select 
*,
(select * from SUCURSAL t
 where t.IdSucursal = up.IdSucursal
FOR XML PATH (''),TYPE) AS 'DetalleTienda'
,
(select * from PERFIL r
 where r.IdPerfil = up.IdPerfil
...
,
 ...
 (select sm.Nombre[NombreSubMenu],sm.Controlador,sm.Vista,sm.Icono,p.Activo
	 from PERMISOS p
	 inner join PERFIL r on r.IdPerfil = p.IdPerfil
	 inner join SUBMENU sm on sm.IdSubMenu = p.IdSubMenu
	 inner join MENU m on m.IdMenu = sm.IdMenu
	 inner join USUARIO u on u.IdPerfil = r.IdPerfil and u.IdUsuario = up.IdUsuario
...
as
begin
 select u.IdUsuario,u.Nombres,u.Apellidos,u.Correo,u.Clave,u.IdSucursal,u.IdPerfil,u.Activo,u.FechaRegistro,r.Descripcion[DescripcionRol],u.Activo from USUARIO u
 inner join PERFIL r on r.IdPerfil = u.IdPerfil
end
...
begin
	...
	IF NOT EXISTS (SELECT * FROM USUARIO WHERE Correo = @Correo)
		...
		insert into USUARIO(Nombres,Apellidos,Correo,Clave,IdSucursal,IdPerfil) values (
...
as
begin
 select IdSucursal,Nombre,RUC,Direccion,Telefono,Activo from SUCURSAL
end

go
--PROCEDIMIENTO PARA GUARDAR SUCURSAL
...
begin
	SET @Resultado = 1
	IF NOT EXISTS (SELECT * FROM SUCURSAL WHERE Nombre = @Nombre)

		insert into SUCURSAL(Nombre,RUC,Direccion,Telefono) values (
...
begin
	SET @Resultado = 1
	IF NOT EXISTS (SELECT * FROM SUCURSAL WHERE Nombre =@Nombre and IdSucursal != @IdTienda)
...
go
--PROCEDIMIENTO PARA ELIMINAR SUCURSAL
...
begin
	...
	inner join SUCURSAL t on t.IdSucursal = u.IdSucursal
	where t.IdSucursal = @IdTienda)

		delete from SUCURSAL where IdSucursal = @IdTienda
...
--PROCEDMIENTO PARA OBTENER PERFILES
CREATE PROC usp_ObtenerPerfiles
as
begin
 select IdPerfil, Descripcion,Activo from PERFIL
end

go

--PROCEDIMIENTO PARA GUARDAR PERFIL
CREATE PROC usp_RegistrarPerfil(
@Descripcion varchar(50),
@Resultado bit output
)as
begin
	SET @Resultado = 1
	IF NOT EXISTS (SELECT * FROM PERFIL WHERE Descripcion = @Descripcion)
	begin
		declare @idrol int = 0
...
--PROCEDIMIENTO PARA MODIFICAR PERFILES
create procedure usp_ModificarPerfiles(
@IdRol int,
...
begin
	SET @Resultado = 1
	IF NOT EXISTS (SELECT * FROM PERFIL WHERE Descripcion =@Descripcion and IdPerfil != @IdRol)
...
create procedure usp_EliminarPerfil(
@IdRol int,
@Resultado bit output
)
as
begin
	...
	IF not EXISTS (select * from USUARIO u
	inner join PERFIL r on r.IdPerfil  = u.IdPerfil
	where r.IdPerfil = @IdRol)
	begin	
		delete from PERMISOS where IdPerfil = @IdRol
		delete from PERFIL where IdPerfil = @IdRol
	end
	ELSE
		SET @Resultado = 0
...
begin
	...
	IF not EXISTS (select top 1 * from PRODUCTO_SUCURSAL PT
...
--PROCEDMIENTO PARA OBTENER PRODUCTO_SUCURSAL
CREATE PROC usp_ObtenerProductoSucursal
as
begin
 select pt.IdProductoSucursal,p.IdProducto,p.Codigo[CodigoProducto], p.Nombre[NombreProducto],p.Descripcion[DescripcionProducto],
 t.IdSucursal,t.RUC,t.Nombre[NombreSucursal] ,t.Direccion[DireccionSucursal],pt.PrecioUnidadCompra,pt.PrecioUnidadVenta,pt.Stock,pt.Iniciado
 from PRODUCTO_SUCURSAL pt
 inner join PRODUCTO p on p.IdProducto = pt.IdProducto
 inner join SUCURSAL t on t.IdSucursal = pt.IdSucursal
end

go


--PROCEDIMIENTO PARA GUARDAR PRODUCTO_SUCURSAL
...
begin
	SET @Resultado = 1
	IF NOT EXISTS (SELECT * FROM PRODUCTO_SUCURSAL WHERE IdProducto = @IdProducto and IdSucursal = @IdTienda)

		insert into PRODUCTO_SUCURSAL(IdProducto,IdSucursal) values (
		@IdProducto,@IdTienda
		)
	ELSE
		SET @Resultado = 0
...
--PROCEDIMIENTO PARA MODIFICAR PRODUCTO_SUCURSAL
...
begin
	SET @Resultado = 1
	IF NOT EXISTS (SELECT * FROM PRODUCTO_SUCURSAL WHERE @IdProductoTienda = @IdProductoTienda and Iniciado = 1 )
		
		update PRODUCTO_SUCURSAL set 
		IdProducto = @IdProducto,
		IdSucursal = @IdTienda
		where IdProductoSucursal = @IdProductoTienda
	ELSE
		SET @Resultado = 0
...
--PROCEDIMIENTO PARA ELIMINAR PRODUCTO_SUCURSAL
...
begin
	...
	IF NOT EXISTS (SELECT * FROM PRODUCTO_SUCURSAL WHERE IdProductoSucursal = @IdProductoTienda and Iniciado = 1 )

		delete from PRODUCTO_SUCURSAL where IdProductoSucursal = @IdProductoTienda
...
--PROCEDIMIENTO PARA STOCK PRODUCTO_SUCURSAL
...
begin
	...
	begin try
		if(@Restar = 1)
			update PRODUCTO_SUCURSAL set Stock = Stock - @Cantidad where IdProducto = @IdProducto and IdSucursal = @IdTienda and Iniciado = 1
		else
			update PRODUCTO_SUCURSAL set Stock = Stock + @Cantidad where IdProducto = @IdProducto and IdSucursal = @IdTienda and Iniciado = 1
	end try
	begin catch
...
begin try
	...
	declare @tiendaproducto table(idtienda int,idproducto int,cantidad int,preciounidadcompra decimal(18,2),preciounidadventa decimal(18,2),totalcosto decimal(18,2))
	 ...
	 IdSucursal = Node.Data.value('(IdTienda)[1]','int'),
	 ...
	 insert into COMPRA(IdUsuario,IdProveedor,IdSucursal,TotalCosto)
	 ...
	 pt.Iniciado = 1
	 from PRODUCTO_SUCURSAL pt
	 inner join @tiendaproducto tp on tp.idtienda = pt.IdSucursal and tp.idproducto = pt.IdProducto
...
(select T.RUC, T.Nombre, T.Direccion from SUCURSAL T
 where T.IdSucursal = c.IdSucursal
...
inner join PROVEEDOR p on p.IdProveedor = c.IdProveedor
inner join SUCURSAL t on t.IdSucursal = c.IdSucursal
where 
...
t.IdSucursal =iif(@IdTienda = 0,t.IdSucursal,@IdTienda) 
...
begin try
	...
	select 
		 IdSucursal = Node.Data.value('(IdTienda)[1]','varchar(50)'),
	...
	insert into VENTA(Codigo,ValorCodigo,IdSucursal,IdUsuario,IdCliente,TipoDocumento,CantidadProducto,CantidadTotal,TotalCosto,ImporteRecibido,ImporteCambio)
...
(select T.RUC, T.Nombre, T.Direccion from SUCURSAL T
 where T.IdSucursal = V.IdSucursal
FOR XML PATH (''),TYPE) AS 'DETALLE_TIENDA',
...
--PROCEDMIENTO PARA REPORTE TIENDA PRODUCTO
 ...
 begin
 select 
 t.RUC[Ruc Sucursal],t.Nombre[Nombre Sucursal],t.Direccion[Direccion Sucursal],
 p.Codigo[Codigo Producto],p.Nombre[Nombre Producto],p.Descripcion[Descripcion Producto],
 pt.Stock[Stock en Sucursal],convert(decimal(10,2),pt.PrecioUnidadCompra)[Precio Compra],convert(decimal(10,2),pt.PrecioUnidadVenta)[Precio Venta]
 from PRODUCTO_SUCURSAL pt
 inner join PRODUCTO p on p.IdProducto = pt.IdProducto
 inner join SUCURSAL t on t.IdSucursal = pt.IdSucursal
 where pt.IdSucursal = iif(@IdTienda = 0,pt.IdSucursal,@IdTienda) and
...
begin
 ...
 t.Nombre[Nombre Sucursal], t.RUC[Ruc Sucursal],
 ...
 from VENTA v
 inner join SUCURSAL t on t.IdSucursal = v.IdSucursal
 inner join USUARIO u on u.IdUsuario = v.IdUsuario
 where 
